...
class MY_Model extends CI_Model
{
    ...
    public function concatStringClob($string)
    {
        $tamStringEmail = mb_strlen($string);
        $string = str_replace("'", "''", $string);
        if($tamStringEmail > Constant::ONETHOUSAND){
            $div = intdiv($tamStringEmail, Constant::ONETHOUSAND);
            $partsString = [];
            for($i = 1; $i <= $div; $i++){
                $partsString[] = substr($string, ($i - 1) * Constant::ONETHOUSAND, Constant::ONETHOUSAND);
            }
            $restDiv = $tamStringEmail % Constant::ONETHOUSAND;
            $partsString[] = substr($string, $div * Constant::ONETHOUSAND, $restDiv);
            return html_entity_decode("to_clob('" . implode("') || to_clob('", $partsString) . "')");
        } else {
            return "to_clob('" . $string . "')";
        }
    }
    ...
    {
        return !empty($term) ? ('fuzzy(' . implode('%) & fuzzy(', explode(' ', preg_replace("/\s{2,}/", ' ', trim(mb_strtoupper(str_replace('-', '', $term), 'UTF-8'))))) . '%)') : false;
    }
}
